...
final class AdminAnnounceViewController: BaseViewController {
    ...
    private let addNotice = UIButton().then {
        $0.backgroundColor = UIColor(rgb: 0xA39288)
        $0.setTitle("+", for: .normal)
        $0.setTitleColor(UIColor(rgb: 0xFFFFFF), for: .normal)
        $0.titleLabel?.font = UIFont(name: "Adelle-Bold", size: 50)
        $0.layer.cornerRadius = 26
        $0.addTarget(self, action: #selector(goToAddNotice), for: .touchUpInside)
    }
    ...
    override func setLayout() {
        ...
        self.addNotice.snp.makeConstraints {
            $0.top.equalTo (self.view).offset(682)
            $0.leading.equalTo(self.view).inset(303)
            //$0.trailing.equalTo(self.view).inset(30)
            $0.height.equalTo(52)
            $0.width.equalTo(52)
        }
    }
    ...
    @objc private func goToNextStep() {
        let controller = AdminProfileViewController()
        navigationController?.pushViewController(controller, animated: true)
    }
    
    @objc private func goToAddNotice() {
        let controller = EditNoticeViewController()
        navigationController?.pushViewController(controller, animated: true)
    }
}
